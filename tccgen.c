...
redo:
	if (tok != TOK_ATTRIBUTE1 && tok != TOK_ATTRIBUTE2 && tok != TOK_ATTRIBUTE1_CN)
		return;
	next();
	skip('(');
	skip('(');
...
static int parse_btype(CType *type, AttributeDef *ad)
{
	...
	while(1) {
		...
		case TOK_RESTRICT2:
		case TOK_RESTRICT3:
		case TOK_RESTRICT1_CN:
			next();
			break;
		case TOK_UNSIGNED:
		case TOK_UNSIGNED_CN:
...
static int post_type(CType *type, AttributeDef *ad, int storage, int td)
{
	...
	if (tok == '(') {
		...
		/* array definition */
		next();
		if (tok == TOK_RESTRICT1 || tok == TOK_RESTRICT1_CN)
			next();
		n = -1;
		t1 = 0;
		if (tok != ']') {
...
ST_FUNC void unary(void)
{
 ...
 push_tokc:
	...
	case TOK___FUNCTION___CN:
		if (!gnu_ext)
			goto tok_identifier;
		/* fall thru */
	case TOK___FUNC__:
	case TOK___FUNC___CN:
		{
			void *ptr;
			int len;
...
static void block(int *bsym, int *csym, int is_expr)
{
	...
	if (tok == TOK_IF || tok == TOK_IF_CN) {
		...
		if (tok == TOK_LABEL || tok == TOK_LABEL_CN) {
			next();
			for(;;) {
				if (tok < TOK_UIDENT)
					expect("label identifier");
		...
		next();
	} else if (tok == TOK_RETURN || tok == TOK_RETURN_CN) {
		next();
		if (tok != ';') {
			gexpr();
		...
		if (tok != '}' || local_scope != 1)
			rsym = gjmp(rsym);
		nocode_wanted |= 0x20000000;
	} else if (tok == TOK_BREAK || tok == TOK_BREAK_CN) {
		/* compute jump */
		if (!bsym)
		...
		*bsym = gjmp(*bsym);
		...
		skip(';');
		nocode_wanted |= 0x20000000;
	} else if (tok == TOK_CONTINUE || tok == TOK_CONTINUE_CN) {
		/* compute jump */
		if (!csym)
		...
		*csym = gjmp(*csym);
		next();
		skip(';');
	} else if (tok == TOK_FOR || tok == TOK_FOR_CN) {
		int e;
	...
	if (tok == TOK_SWITCH || tok == TOK_SWITCH_CN) {
	...
	if (tok == TOK_CASE || tok == TOK_CASE_CN) {
	...
	if (tok == TOK_GOTO || tok == TOK_GOTO_CN) {
		next();
		if (tok == '*' && gnu_ext) {
			/* computed goto */
			next();
		...
		skip(';');
	} else if (tok == TOK_ASM1 || tok == TOK_ASM2 || tok == TOK_ASM3 || tok == TOK_ASM1_CN) {
		asm_instr();
	} else {
		b = is_label();
		if (b) {
...
static int decl0(int l, int is_for_loop_init, Sym *func_sym)
{
	...
	while (1) {
		...
		while (1) { /* iterate thru each declaration */
			...
			if (gnu_ext && (tok == TOK_ASM1 || tok == TOK_ASM2 || tok == TOK_ASM3 || tok == TOK_ASM1_CN)) {
