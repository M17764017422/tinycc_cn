		...
		case '#':
			...
			if (start_of_line) {
				...
				if (a == 0 && 
					...
					goto the_end;
				if (tok == TOK_IF || tok == TOK_IF_CN || tok == TOK_IFDEF || tok == TOK_IFDEF_CN || tok == TOK_IFNDEF || tok == TOK_IFNDEF_CN)
					a++;
				else if (tok == TOK_ENDIF || tok == TOK_ENDIF_CN)
					a--;
...
static int expr_preprocess(void)
{
	...
	while (tok != TOK_LINEFEED && tok != TOK_EOF) {
		next(); /* do macro subst */
		if (tok == TOK_DEFINED || tok == TOK_DEFINED_CN) {
			next_nomacro();
			t = tok;
			if (t == '(') 
				next_nomacro();
	...
	Sym *s)
{
	...
	/* special macros */
	if (tok == TOK___LINE__ || tok == TOK___LINE___CN || tok == TOK___COUNTER__ || tok == TOK___COUNTER___CN) {
		t = (tok == TOK___LINE__ || tok == TOK___LINE___CN) ? file->line_num : pp_counter++;
		snprintf(buf, sizeof(buf), "%d", t);
		cstrval = buf;
		t1 = TOK_PPNUM;
		goto add_cstr1;
	} else if (tok == TOK___FILE__ || tok == TOK___FILE___CN) {
		cstrval = file->filename;
		goto add_cstr;
	} else if (tok == TOK___DATE__ || tok == TOK___DATE___CN || tok == TOK___TIME__ || tok == TOK___TIME___CN) {
		time_t ti;
		struct tm *tm;

		time(&ti);
		tm = localtime(&ti);
		if (tok == TOK___DATE__ || tok == TOK___DATE___CN) {
...
static void pp_debug_defines(TCCState *s1)
{
	...
	if (t == TOK_DEFINE || t == TOK_DEFINE_CN) {
		define_print(s1, v);
	} else if (t == TOK_UNDEF || t == TOK_UNDEF_CN) {
		fprintf(fp, "#undef %s\n", vs);
